...
const registrationSchema = z
    .object({
        ...
        email: z.string().email(tokens.register.error.email.invalid),
        password: z.string().min(8, tokens.register.error.password.minLength),
...
const isFormValid = computed(() => {
    const hasAllFields =
        !isValueEmpty(formData.username) &&
        !isValueEmpty(formData.email) &&
        !isValueEmpty(password.value) &&
        !isValueEmpty(formData.confirmPassword);
...
const handleSubmit = async () => {
    ...
    try {
        ...
        switch (mode) {
            case 'update':
                await api().user().update(formData.username, formData.email, formData.password);
...
<template>
    <div class="registration-container">
        <div class="registration-card">
            <h1 class="title">{{ $t(tokens[mode].title) }}</h1>
            <p class="subtitle">{{ $t(tokens[mode].subtitle) }}</p>
            ...
            <form @submit.prevent="handleSubmit" class="form" v-if="!submitSuccess">
                ...
                <div class="form-group">
                    <label for="username" class="label"
                        >{{ $t(tokens.user.username) }} <span class="red">*</span></label
                    >
                    <input
                        id="username"
                        type="text"
                ...
                <div class="form-group">
                    <label for="email" class="label"
                        >{{ $t(tokens.user.email) }} <span class="red">*</span></label
                    >
                    <input
                        id="email"
                        type="text"
                ...
                <div class="form-group">
                    <label for="password" class="label"
                        >{{ $t(tokens.user.password) }} <span class="red">*</span></label
                    >
                    <input
                        id="password"
                        type="password"
                    ...
                    <div v-if="password" class="password-strength">
                        ...
                        <div class="strength-info">
                            <span class="strength-label" :style="{ color: strengthResult.color }">
                ...
                <div class="form-group">
                    <label for="confirmPassword" class="label"
                        >{{ $t(tokens.register.confirmPassword) }} <span class="red">*</span></label
                    >
                    <input
                        id="confirmPassword"
                        type="password"
                ...
                >
                    ...
                    {{ isSubmitting ? $t(tokens.register.submitting) : $t(tokens[mode].submit) }}
                </button>
            </form>
